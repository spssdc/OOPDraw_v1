using System;
using System.Collections.Generic;
using System.Text;
using System.Drawing;

namespace OOPDraw
{
    // Declare new class type called Line
    public class Line
    {
        // Set up class attirbutes as publicly accesible, but only privately changeable
        public Pen Pen { get; private set; }
        public int X1 { get; private set; }
        public int Y1 { get; private set; }
        public int X2 { get; private set; }
        public int Y2 { get; private set; }

        // class constructor for Line taking parameters for the pen and coordinates
        // uses the parameters to modify the privately set attributes for the instance
        // it is public as you have to be able to call from outside the class
        // there is no return type
        public Line(Pen p, int x1, int y1, int x2, int y2)
        {
            Pen = p;
            X1 = x1;
            Y1 = y1;
            X2 = X2;
            Y2 = Y2;
        }

        // second version of the constructor, called with fewer parameters,
        // for a zero length line (at the start of drag/click operation)
        public Line(Pen p, int x1, int y1) : this(p, x1, y1, x1, y1)
        {

        }

        // publicly invokable method that can be applied to a line
        public void Draw(Graphics g)
        {
            g.DrawLine(Pen, X1, Y1, X2, Y2);
        }

        // publicly invokable method that can be applied to a line
        public void GrowTo(int x2, int y2)
        {
            X2 = x2;
            Y2 = y2;
        }
    }
}
